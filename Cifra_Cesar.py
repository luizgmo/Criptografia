def cifra_de_cesar(frase, deslocamento, criptografar=True):
    resultado = ""
    for letra in frase:
        if letra.isalpha():
...
def main():
    while True:
        ...
        if opcao == '1':
            ...
            frase_criptografada = cifra_de_cesar(frase, chave)
            print("")
            print("Frase criptografada:", frase_criptografada)
            
            with open("resultado_criptografia_cesar.txt", "a") as arquivo:
                arquivo.write(f"Frase criptografada (chave={chave}): {frase_criptografada}\n")
                
        elif opcao == '2':
            frase = input("Digite a frase que deseja descriptografar: ")
            chave = int(input("Digite sua chave: "))
            frase_descriptografada = cifra_de_cesar(frase, chave, criptografar=False)
            print("")
...
if __name__ == "__main__":
    with open("resultado_criptografia.txt", "w") as arquivo:
        arquivo.write("Resultados da Criptografia de César\n\n")
    main()
